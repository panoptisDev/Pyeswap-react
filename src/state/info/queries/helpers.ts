...
import { MintResponse, SwapResponse, BurnResponse, TokenDayData, PairDayData, PyeDayData } from './types'

export const filterToken0 = (row: any, address: string) => {
  return row?.token0Address && row?.token0Address.toLowerCase() === address.toLowerCase();
}

export const filterToken1 = (row: any, address: string) => {
  return row?.token1Address && row?.token1Address.toLowerCase() === address.toLowerCase();
}
...
export const mapDayData = (tokenDayData: TokenDayData | PyeDayData): ChartEntry => ({
