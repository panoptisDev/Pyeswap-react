...
import { PyeDayDatasResponse } from '../types'
import { fetchChartData, mapDayData } from '../helpers'

/**
 * Data for displaying Liquidity and Volume charts on Overview page
 */
const PYE_DAY_DATAS = gql`
  query overviewCharts($startTime: Int!, $skip: Int!) {
    pyeDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      date
      dailyVolumeUSD
      totalLiquidityUSD
    }
...
const getOverviewChartData = async (skip: number): Promise<{ data?: ChartEntry[]; error: boolean }> => {
  try {
    const { pyeDayDatas } = await request<PyeDayDatasResponse>(INFO_CLIENT, PYE_DAY_DATAS, {
      startTime: PCS_V2_START,
      skip,
    })
    const data = pyeDayDatas.map(mapDayData)
