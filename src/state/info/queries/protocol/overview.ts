...
const formatPyeFactoryResponse = (rawPyeFactory?: PyeFactory) => {
  if (rawPyeFactory) {
    return {
      totalTransactions: parseFloat(rawPyeFactory.totalTransactions),
      totalVolumeUSD: parseFloat(rawPyeFactory.totalVolumeUSD),
      totalLiquidityUSD: parseFloat(rawPyeFactory.totalLiquidityUSD),
    }
  }
  return null
}
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const overviewData = formatPyeFactoryResponse(data?.pyeFactories?.[0])
      const overviewData24 = formatPyeFactoryResponse(data24?.pyeFactories?.[0])
      const overviewData48 = formatPyeFactoryResponse(data48?.pyeFactories?.[0])
