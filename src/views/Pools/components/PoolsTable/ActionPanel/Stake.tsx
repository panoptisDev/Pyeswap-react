...
import VaultStakeModal from '../../AppleVaultCard/VaultStakeModal'
...
const Staked: React.FunctionComponent<StackedActionProps> = ({ pool, userDataLoaded }) => {
  ...
  const stakedAutoDollarValue = getBalanceNumber(appleAsBigNumber.multipliedBy(stakingTokenPrice), stakingToken.decimals)
  ...
  const [onPresentVaultUnstake] = useModal(<VaultStakeModal stakingMax={appleAsBigNumber} pool={pool} isRemovingStake />)
  ...
  if (!account) {
    return (
      <ActionContainer>
        ...
        <ActionContent>
          <ConnectWalletButton variant='primary' width="100%" />
        </ActionContent>
      </ActionContainer>
    )
  }
  ...
  if (isNotVaultAndHasStake || isVaultWithShares) {
    return (
      <ActionContainer>
        <ActionTitles>
          <Text fontSize="13px" bold color="secondary" as="span">
            {stakingToken.symbol}
          </Text>
          <Text fontSize="13px" bold color="textSubtle" as="span" ml='4px'>
        ...
        <ActionContent>
          <Flex flex="1" pt="16px" flexDirection="column" alignSelf="flex-start">
            <Balance
              ...
              fontSize="26px"
              decimals={5}
              value={vaultKey ? appleAsNumberBalance : stakedTokenBalance}
            />
            <Balance
              fontSize="13px"
              display="inline"
          ...
          <IconButtonWrapper>
            <IconButton variant="light" onClick={onUnstake} mr="6px">
