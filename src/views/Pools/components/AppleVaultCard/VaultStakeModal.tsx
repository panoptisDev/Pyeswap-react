...
import { fetchAppleVaultUserData } from 'state/pools'
...
import { convertAppleToShares, convertSharesToApple } from '../../helpers'
...
const VaultStakeModal: React.FC<VaultStakeModalProps> = ({
  ...
  const usdValueStaked = new BigNumber(stakeAmount).times(applePriceBusd)
  const formattedUsdValueStaked = applePriceBusd.gt(0) && stakeAmount ? formatNumber(usdValueStaked.toNumber()) : ''
  ...
  const handleWithdrawal = async (convertedStakeAmount: BigNumber) => {
    setPendingTx(true)
    const shareStakeToWithdraw = convertAppleToShares(convertedStakeAmount, pricePerFullShare)
    // trigger withdrawAll function if the withdrawal will leave 0.000001 APPLE or less
  ...
  if (showRoiCalculator) {
    return (
      <RoiCalculatorModal
        ...
        stakingTokenBalance={appleAsBigNumber.plus(stakingMax)}
  ...
  return (
    ...
    >
      <Flex alignItems="center" justifyContent="space-between" mb="8px">
        <Text fontSize='15px' bold>{isRemovingStake ? t('Unstake') : t('Stake')}:</Text>
      ...
      <BalanceInput
        ...
        currencyValue={applePriceBusd.gt(0) && `~${formattedUsdValueStaked || 0} USD`}
      ...
      {!isRemovingStake && (
        <Flex mt="24px" alignItems="center" justifyContent="space-between">
          <Text fontSize='13px' mr="8px" color="textSubtle">
