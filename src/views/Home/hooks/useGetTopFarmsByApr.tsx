...
import { useFarms, usePriceAppleBusd } from 'state/farms/hooks'
...
const useGetTopFarmsByApr = (isIntersecting: boolean) => {
  ...
  useEffect(() => {
    const getTopFarmsByApr = (farmsState: DeserializedFarm[]) => {
      ...
      const farmsWithApr: FarmWithStakedValue[] = farmsWithPrices.map((farm) => {
        ...
        const { appleRewardsApr, lpRewardsApr } = getFarmApr(
          farm.poolWeight,
          applePriceBusd,
          totalLiquidity,
          farm.lpAddresses[ChainId.MAINNET],
        )
        return { ...farm, apr: appleRewardsApr, lpRewardsApr }
      })
  ...
  }, [setTopFarms, farms, fetchStatus, applePriceBusd, topFarms])
