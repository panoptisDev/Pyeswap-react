...
import { Text, Flex, Box, Radio, Skeleton, LinkExternal, ArrowForwardIcon, ArrowBackIcon, Heading } from 'uikit'
...
const DataRow: React.FC<{ transaction: Transaction }> = ({ transaction }) => {
  ...
  return (
    <ResponsiveGrid style={{ background: theme.colors.background }}>
      ...
      <Text color="textSubtle">${formatAmount(transaction.amountUSD)}</Text>
      <Text>
        <Text color="textSubtle">{`${formatAmount(abs0)} ${transaction.token0Symbol}`}</Text>
      </Text>
      <Text>
        <Text color="textSubtle">{`${formatAmount(abs1)} ${transaction.token1Symbol}`}</Text>
      </Text>
      <LinkExternal href={getBscScanLink(transaction.sender, 'address')}>
        {truncateHash(transaction.sender)}
      </LinkExternal>
      <Text color="textSubtle">{formatDistanceToNowStrict(parseInt(transaction.timestamp, 10) * 1000)}</Text>
    </ResponsiveGrid>
  )
}
...
const TransactionTable: React.FC<{
  ...
  const renderPagination = (currentPage: number, totalPages: number) => {
    for (let i = 1; i <= totalPages; i++) {
      return <Text color={i === currentPage ? 'success' : 'secondary'}>{i}</Text>
    }
  }

  return (
    <Wrapper>
      <Flex mt="25px" mb="25px" alignItems="center" justifyContent="space-between">
        <Flex>
