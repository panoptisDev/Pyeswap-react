...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://pye.net - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/pye.net)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://pye.net'}
          color={text === 'https://pye.net' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
