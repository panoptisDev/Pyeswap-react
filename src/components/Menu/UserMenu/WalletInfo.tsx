...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowBnbBalance, onDismiss }) => {
  ...
  const { balance: pyeBalance, fetchStatus: pyeFetchStatus } = useTokenBalance(tokens.pye.address)
  ...
  return (
    <>
      ...
      <Flex alignItems="center" justifyContent="space-between" mb="24px">
        <Text color="textSubtle">{t('PYE Balance')}</Text>
        {pyeFetchStatus !== FetchStatus.SUCCESS ? (
          <Skeleton height="22px" width="60px" />
        ) : (
          <Text>{getFullDisplayBalance(pyeBalance, 18, 3)}</Text>
        )}
      </Flex>
